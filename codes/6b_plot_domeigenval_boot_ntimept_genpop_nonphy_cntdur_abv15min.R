...
fname_RData_cntmat_boot = "domeigen_cntmat_ntimept_nboot1000_genpop_nonphy_cntdur_abv15min.RData";
...
COLS_pick = c("All"="blue3", "Physical"="red2", "Non-physical" = "darkgreen")
PCHS_pick = c("All"=16, "Physical"=18, "Non-physical" = 17)

fname_excelout = sprintf("ratio_domeigenval_boot_genpop_%s_nonphy_cntdur_abv15min.xlsx", update_outputdate())
...
YMAX = 5 # max(stat_ratio_domeigenval_plot[iplot_vec,,])
YMAX = max(pretty(c(0,YMAX)))
YMAX = ceiling(YMAX*2)/2 # per 0.5
yaxis_at = seq(0, YMAX, by=1)
...
for (iplot in iplot_vec){
	...
	iplot_xtime_shift = seq(from=-1, to=1, length.out=length(iplot_vec))

	iplot_xtime_CI = iplot_xtime + 0.15*iplot_xtime_shift[iplot]
	cex_pch = ifelse(max(iplot_vec)==2, 0.8, 0.7)
	
	arrow_use = arrow(angle=90, ends="both", length=unit(0.025, "npc"))
	arrow_use = NULL;
	for (itime in 1:num_date_byphase){
		grid.points(iplot_xtime_CI[itime], y=stat_ratio_domeigenval_plot[iplot, itime, 'median'], default.units = 'native', pch=PCHS_iplot, gp = gpar(col=COLS_iplot, cex=cex_pch))
		if (itime !=1){
