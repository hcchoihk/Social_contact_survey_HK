...
contactdur_abv15min_YN = TRUE;
# prop.table( table(chk_contact_base[, c("duration_multi")]) )
...
boot_TT_chk = future_sapply(1:num_boot, simplify=FALSE, function(iboot) {
	...
	if (contactdur_abv15min_YN==TRUE){
		chk_contact_iboot = subset(chk_contact_iboot, subset=chk_contact_iboot$duration_multi>=3)
	} else if (contactdur_abv15min_YN==FALSE) {
		chk_contact_iboot = subset(chk_contact_iboot, subset=chk_contact_iboot$duration_multi<=2)
	}	
...
fname_out_RData = sprintf("domeigen_cntmat_ntimept_nboot%d_genpop_nonphy_cntdur_abv15min.RData", num_boot)
